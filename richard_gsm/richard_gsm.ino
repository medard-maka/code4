...
char phone_no[]="+243982125697"; //change +92 with country code and 3378655465 with phone number to sms
...
pinMode(bt_M, INPUT); // declare bt_M as input
...
else if(inchar=='A'){delay(10); inchar=GSM.read(); 
     if(inchar=='L'){delay(10); inchar=GSM.read();
     if(inchar=='M'){delay(10); inchar=GSM.read();
...
if(digitalRead (bt_M) == 1){sendSMS(phone_no,"Tentative d'intruison veuillez verifier la camera");
